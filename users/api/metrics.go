...
func (ms *metricsMiddleware) UnassignUserFromGroup(ctx context.Context, token, userID, groupID string) error {
	defer func(begin time.Time) {
	...
	return ms.svc.UnassignUserFromGroup(ctx, token, userID, groupID)
}
