...
func NewClient(tracer opentracing.Tracer, conn *grpc.ClientConn, timeout time.Duration) mainflux.UserServiceClient {
	return &grpcClient{
		issue: kitot.TraceClient(tracer, "issue")(kitgrpc.NewClient(
			conn,
			"mainflux.UserService",
			"Membership",
			ecnodeMembershipRequest,
			decodeIssueResponse,
			mainflux.UserID{},
		).Endpoint()),
		timeout: timeout,
...
func (client grpcClient) Membership(ctx context.Context, req *mainflux.IssueReq, _ ...grpc.CallOption) (*mainflux.Token, error) {
	ctx, close := context.WithTimeout(ctx, client.timeout)
	defer close()

	res, err := client.Membership(ctx, issueReq{issuer: req.GetIssuer(), keyType: req.Type})
	if err != nil {
		return nil, err
	}
