...
func TestLogin(t *testing.T) {
	...
	client := ts.Client()
	auth := mocks.NewAuthService(map[string]string{user.Email: user.Email})
	tkn, _ := auth.IssueKey(context.Background(), &mainflux.IssueReq{Id: user.ID, Email: user.Email, Type: 0})
	token := tkn.GetValue()
...
func TestPasswordReset(t *testing.T) {
	...
	tkn, err := auth.IssueKey(context.Background(), &mainflux.IssueReq{Id: user.ID, Email: user.Email, Type: 0})
