...
import (
	...
	groupsAPI "github.com/mainflux/mainflux/auth/api/groups/http"
...
func MakeHandler(svc auth.Service, tracer opentracing.Tracer) http.Handler {
	...
	mux.Get("/groups/:groupID/children", kithttp.NewServer(
		kitot.TraceServer(tracer, "list_children_groups")(groupsAPI.ListChildrenEndpoint(svc)),
		groupsAPI.DecodeListGroupsRequest,
		encodeResponse,
		opts...,
	))
	...
	mux.Get("/groups/:groupID/members", kithttp.NewServer(
		...
		groupsAPI.DecodeListMembersRequest,
		encodeResponse,
		opts...,
	))

	mux.Get("/members/:memberID/groups", kithttp.NewServer(
		kitot.TraceServer(tracer, "list_memberships")(groupsAPI.ListMemberships(svc)),
		groupsAPI.DecodeListMembershipsRequest,
		encodeResponse,
		opts...,
	))
